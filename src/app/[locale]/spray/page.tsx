    ...
    <div className="max-w-7xl mx-auto py-10 px-4 text-white">
      <div className="flex space-x-4 overflow-x-auto border-b border-blue-500 pb-2 mb-8">
        {weapons.map((weapon) => (
          <button
            key={weapon.id}
            onClick={() => refs[weapon.id]?.current?.scrollIntoView({ behavior: "smooth" })}
            ...
            <div className="md:w-1/2 space-y-4 mb-6 md:mb-0">
              ...
              <div className="space-y-4 mt-4">
                <div>
                  ...
                  <p className="text-sm text-green-400">{weapon.recoilHeightDiff}° ({((weapon.recoilHeightDiff / weapon.recoilHeight) * 100).toFixed(2)}%) less than CS:GO</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Recoil Width</p>
                  <p className="text-xl font-bold">{weapon.recoilWidth.toFixed(2)}°</p>
                  <p className="text-sm text-green-400">{weapon.recoilWidthDiff}° ({((weapon.recoilWidthDiff / weapon.recoilWidth) * 100).toFixed(2)}%) less than CS:GO</p>
                </div>
                <div>
                  <p className="text-sm text-gray-400">Total Recoil Area</p>
                  <p className="text-xl font-bold">{weapon.totalRecoil.toFixed(2)}°</p>
                  <p className="text-sm text-green-400">{weapon.totalRecoilDiff}° ({((weapon.totalRecoilDiff / weapon.totalRecoil) * 100).toFixed(2)}%) less than CS:GO</p>
                </div>
              </div>
            </div>
